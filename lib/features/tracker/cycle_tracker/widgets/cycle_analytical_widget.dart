          ...
          Padding(
            ...
            child: Row(
              ...
              children: [
                ...
                _buildStatMini("VOLUME", widget.data["volume"]?[_currentIndex], Colors.orangeAccent, isCompact),
              ],
            ),
          ),
...
class CycleComparisonWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(
      builder: (context, constraints) {
        final bool isCompact = constraints.maxWidth < kMobileBreakpoint;
        return Container(
          ...
          child: Column(
            ...
            children: [
              Row(
                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: [
                  _buildPointPicker("POINT A", idx1, idx2, dates, onPointAChanged, context, isCompact: isCompact),
                  SizedBox(width: isCompact ? 16.w : 12.0),
                  _buildPointPicker("POINT B", idx2, idx1, dates, onPointBChanged, context, isEnd: true, isCompact: isCompact),
                ],
              ),
              if (idx1 != null && idx2 != null) ...[
                SizedBox(height: isCompact ? 24.h : 20.0),
                _buildComparisonDetails(isCompact),
              ] else ...[
  ...
  Widget _buildPointPicker(String title, int? selectedIdx, int? otherIdx, List<DateTime> dates, Function(int?) onChanged, BuildContext context, {bool isEnd = false, required bool isCompact}) {
        ...
        children: [
          Row(
            ...
            children: [
              ...
              Text(title, style: AppTextStyles.labelSmall.copyWith(color: selectedIdx != null ? AppColors.crimson : AppColors.textSecondary.withValues(alpha : 0.4), fontSize: isCompact ? 10.sp : 8.0, fontWeight: FontWeight.w500)),
              if (selectedIdx != null && !isEnd) ...[
                SizedBox(width: isCompact ? 8.w : 6.0),
                GestureDetector(
                  onTap: () => onChanged(null),
          ...
          GestureDetector(
            onTap: () => _showPicker(context, selectedIdx, otherIdx, dates, onChanged, isCompact),
            child: Container(
              ...
              child: Row(
                ...
                children: [
                  ...
                  Flexible(child: Text(selectedIdx != null ? labels[selectedIdx] : "SET POINT", overflow: TextOverflow.ellipsis, style: AppTextStyles.labelSmall.copyWith(fontSize: isCompact ? 11.sp : 9.0, color: selectedIdx != null ? Colors.white : AppColors.textSecondary.withValues(alpha : 0.4)))),
                ],
              ),
            ),
          ),
  ...
  void _showPicker(BuildContext context, int? current, int? other, List<DateTime> dates, Function(int?) onChanged, bool isCompact) async {
              ...
              children: [
                ...
                Expanded(
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
              Text("SELECT RECORDING", style: AppTextStyles.h3.copyWith(fontSize: isCompact ? 20.sp : 16.0)),
              Text("CHOOSE A DATE FROM YOUR HISTORY", style: AppTextStyles.labelSmall.copyWith(color: AppColors.textSecondary.withValues(alpha : 0.5), letterSpacing: 1, fontSize: isCompact ? 11.sp : 9.0)),
                    ],
                  ),
                ),
              ],
                        ...
                        final int? timeResult = await showModalBottomSheet<int>(
                          ...
                          builder: (ctx) => Container(
                            ...
                            child: Column(
                              mainAxisSize: MainAxisSize.min,
                              children: [
                                Text("SELECT TIME", style: AppTextStyles.labelMedium.copyWith(color: AppColors.crimson, letterSpacing: 1.2, fontSize: isCompact ? 13.sp : 11.0)),
                                ...
                                ...indices.map((idx) {
                                  ...
                                  return ListTile(
                                    ...
                                    title: Text(
                                      ...
                                      style: AppTextStyles.labelSmall.copyWith(color: isCurrent ? Colors.white : (isOther ? AppColors.textSecondary.withValues(alpha : 0.5) : AppColors.textSecondary), fontSize: isCompact ? 11.sp : 9.0),
                                    ),
                        ...
                        children: [
                          ...
                          if (indices.length > 1)
                            Container(
                              ...
                              decoration: BoxDecoration(color: AppColors.white.withValues(alpha : 0.05), borderRadius: BorderRadius.circular(isCompact ? 8.r : 6.0)),
                              child: Text("${indices.length} LOGS", style: AppTextStyles.labelSmall.copyWith(fontSize: isCompact ? 9.sp : 7.0, color: AppColors.textSecondary.withValues(alpha : 0.5))),
                            ),
                        ],
                      ),
                    ),
  ...
  Widget _buildComparisonDetails(bool isCompact) {
    ...
    for (var m in metrics) {
      ...
      if (v1 != null && v2 != null) {
        items.add(_buildMetricComparison(m['label'] as String, v1, v2, m['unit'] as String, m['color'] as Color, isCompact));
      }
    }
      ...
      return Center(
        child: Padding(
          ...
          child: Text(
            ...
            style: AppTextStyles.labelSmall.copyWith(color: AppColors.textSecondary.withValues(alpha : 0.3), fontSize: 7.0)
          ),
        ),
      );
    }
  ...
  Widget _buildMetricComparison(String label, double v1, double v2, String unit, Color color, bool isCompact) {
    final delta = v2 - v1;
      ...
      child: Column(
        children: [
          Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
            Text(label, style: AppTextStyles.labelSmall.copyWith(color: color, fontWeight: FontWeight.w500, fontSize: isCompact ? 13.sp : 11.0, letterSpacing: 1)),
              Row(
                children: [
                  ...
                  SizedBox(width: 4.0),
                  Text("${delta >= 0 ? '+' : ''}${percent.toStringAsFixed(1)}%", style: AppTextStyles.labelSmall.copyWith(color: delta >= 0 ? Colors.greenAccent : Colors.redAccent, fontWeight: FontWeight.w500, fontSize: isCompact ? 13.sp : 11.0)),
                ],
              ),
            ],
          ),
          ...
          Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
              _valItem("Point A", v1, unit, isCompact),
              _valItem("Point B", v2, unit, isCompact),
              _valItem("Difference", delta, unit, isCompact, isDelta: true),
            ],
          ),
        ],
      ),
  ...
  Widget _valItem(String l, double v, String u, bool isCompact, {bool isDelta = false}) {
    return Column(
      children: [
        Text(l, style: AppTextStyles.labelSmall.copyWith(color: AppColors.textSecondary.withValues(alpha : 0.4), fontSize: isCompact ? 10.sp : 8.0, fontWeight: FontWeight.w500)), 
        SizedBox(height: 2.0),
        Text("${v >= 0 && isDelta ? '+' : ''}${v.toStringAsFixed(1)}$u", style: AppTextStyles.labelMedium.copyWith(color: Colors.white, fontWeight: FontWeight.w500, fontSize: isCompact ? 16.sp : 13.0)), 
      ],
    );
  }
}
