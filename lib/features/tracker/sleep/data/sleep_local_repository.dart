...
class SleepLocalRepository {
  ...
  Future<void> insertLog(SleepLog log) async {
    final db = await _getDatabase();
    await db.insert('sleep_logs', log.toMap(), conflictAlgorithm: ConflictAlgorithm.replace);
  }
  ...
  Future<List<SleepLog>> getAllLogs() async {
    final db = await _getDatabase();
    final List<Map<String, dynamic>> maps = await db.query('sleep_logs', orderBy: 'bedtime DESC');
  ...
  Future<List<SleepLog>> getUnsyncedLogs() async {
    final db = await _getDatabase();
    final List<Map<String, dynamic>> maps = await db.query('sleep_logs', where: 'is_synced = 0');
    return maps.map((map) => SleepLog.fromMap(map)).toList();
  }

  Future<void> markLogSynced(String id) async {
    final db = await _getDatabase();
    await db.update('sleep_logs', {'is_synced': 1}, where: 'id = ?', whereArgs: [id]);
  }
}
